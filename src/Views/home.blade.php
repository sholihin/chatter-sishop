...
<link href="{{ url('/vendor/sholihin/chatter/assets/vendor/spectrum/spectrum.css') }}" rel="stylesheet">
<link href="{{ url('/vendor/sholihin/chatter/assets/css/chatter.css') }}" rel="stylesheet">
@if($chatter_editor == 'simplemde')
<link href="{{ url('/vendor/sholihin/chatter/assets/css/simplemde.min.css') }}" rel="stylesheet">
@elseif($chatter_editor == 'trumbowyg')
<link href="{{ url('/vendor/sholihin/chatter/assets/vendor/trumbowyg/ui/trumbowyg.css') }}" rel="stylesheet">
<style>
	.trumbowyg-box,
	.trumbowyg-editor {
		margin: 0px auto;
...
<div id="chatter" class="chatter_home">
	...
	<div id="chatter_hero">
		...
		@else
		<h1>{{ __('Forum') }}</h1>
		<p>{{ __('Tanyakan Setiap Kendala Mu Disini. ') }}</p>
		@endif
	</div>
					...
					<li>
						<a class="discussion_list"
							...
							<div class="chatter_avatar">
								...
								@if($discussion->user->{$db_field})
								<img
									src="{{ Config::get('chatter.user.relative_url_to_image_assets') . $discussion->user->{$db_field}  }}">
								@else
								<span class="chatter_avatar_circle"
									style="background-color:#<?= \Sholihin\Chattersishop\Helpers\ChatterHelper::stringToColorCode($discussion->user->{Config::get('chatter.user.database_field_with_user_name')}) ?>">
							...
							<div class="chatter_right">
								...
								<div class="chatter_count"><i class="chatter-bubble"></i>
									{{ $discussion->postsCount[0]->total }}</div>
							</div>
...
<div id="new_discussion">
	...
	<form id="chatter_form_editor"
		action="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
		<div class="row">
			<div class="col-md-7">
				<!-- TITLE -->
...
@if( $chatter_editor == 'tinymce' || empty($chatter_editor) )
<script src="{{ url('/vendor/sholihin/chatter/assets/vendor/tinymce/tinymce.min.js') }}"></script>
<script src="{{ url('/vendor/sholihin/chatter/assets/js/tinymce.js') }}"></script>
<script>
...
@elseif($chatter_editor == 'simplemde')
<script src="{{ url('/vendor/sholihin/chatter/assets/js/simplemde.min.js') }}"></script>
<script src="{{ url('/vendor/sholihin/chatter/assets/js/chatter_simplemde.js') }}"></script>
@elseif($chatter_editor == 'trumbowyg')
<script src="{{ url('/vendor/sholihin/chatter/assets/vendor/trumbowyg/trumbowyg.min.js') }}"></script>
<script
	src="{{ url('/vendor/sholihin/chatter/assets/vendor/trumbowyg/plugins/preformatted/trumbowyg.preformatted.min.js') }}">
</script>
<script src="{{ url('/vendor/sholihin/chatter/assets/js/trumbowyg.js') }}"></script>
@endif

<script src="{{ url('/vendor/sholihin/chatter/assets/vendor/spectrum/spectrum.js') }}"></script>
<script src="{{ url('/vendor/sholihin/chatter/assets/js/chatter.js') }}"></script>
<script>
	$('document').ready(function(){
